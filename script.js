    ...
    static searchTemple(location, modifier) {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                const adjustedThreshold = 0.2 + modifier;
...
async function handleDigChoice(location) {
    try {
        ...
        console.error("敲开地面操作失败：", error);
        // 显示敲开地面失败的提示信息并切换到相应失败场景图片
        messageContainer.innerHTML = '';
        showMessage("敲开地面的声音引来了守卫，被守卫击败");
        // 设置失败标记为true
        hasFailed = true;
        // 切换到相应的失败场景图片
        currentSceneImg.src = 'temple_guard.jpg';
...
function getPlayerChoice() {
    return new Promise((resolve) => {
        ...
        document.addEventListener('click', function (event) {
            if (event.target.textContent === "在神庙内走动") {
                resolve(1);
            } else if (event.target.textContent === "敲开地面观察") {
                resolve(2);
            }
        });
    });
